...
func (k Keeper) EthCall(c context.Context, req *types.EthCallRequest) (*types.MsgEthereumTxResponse, error) {
	...
	res, err := k.ApplyMessageWithConfig(c, ctx, msg, nil, false, cfg, txConfig)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
...
func (k Keeper) EstimateGas(c context.Context, req *types.EthCallRequest) (*types.EstimateGasResponse, error) {
	...
	executable := func(gas uint64) (vmerror bool, rsp *types.MsgEthereumTxResponse, err error) {
		...
		// pass false to not commit StateDB
		rsp, err = k.ApplyMessageWithConfig(c, ctx, msg, nil, false, cfg, txConfig)
		if err != nil {
...
func (k Keeper) TraceTx(c context.Context, req *types.QueryTraceTxRequest) (*types.QueryTraceTxResponse, error) {
	...
	for i, tx := range req.Predecessors {
		...
		rsp, err := k.ApplyMessageWithConfig(c, ctx, msg, types.NewNoOpTracer(), true, cfg, txConfig)
		if err != nil {
			continue
		}
...
func (k *Keeper) traceTx(
	...
	res, err := k.ApplyMessageWithConfig(ctx.Context(), ctx, msg, tracer, commitMessage, cfg, txConfig)
	if err != nil {
		return nil, 0, status.Error(codes.Internal, err.Error())
	}
